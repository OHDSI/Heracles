define(["d3", "jquery", "lodash"], function (d3, $, _) {
	...
	function mapConceptData(data) {
        ...
        if (data instanceof Array) {
            ...
            $.each(data, function() {
                var datum = {}
                datum.id = (+this.CONCEPT_ID|| this.CONCEPT_NAME);
                datum.label = this.CONCEPT_NAME;
                datum.value = +this.COUNT_VALUE;
                result.push(datum);
            });
        }
		else if (data.COUNT_VALUE instanceof Array) // multiple rows, each value of each column is in the indexed properties.
		{
